...
(async () => {
  ...
  try {
    ...
    const cli = `npx mapshaper -i "${input}" ${dissolvePart} -simplify visvalingam ${simplify}% keep-shapes -o format=geojson precision=0.0001 ${tmpOut}`;
    if (process.env.DEBUG_OVERLAY) console.log('[buildOverlay] CLI:', cli);
  ...
  if (lodPercents) {
    ...
    try {
      ...
      const cliMid = `npx mapshaper -i "${input}" ${dissolvePart} -simplify visvalingam ${midPct}% keep-shapes -o format=geojson precision=0.0001 ${tmpMid}`;
      const cliLow = `npx mapshaper -i "${input}" ${dissolvePart} -simplify visvalingam ${lowPct}% keep-shapes -o format=geojson precision=0.0001 ${tmpLow}`;
